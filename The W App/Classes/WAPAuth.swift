...
enum WAPAuth {
    ...
    static func signUp(email: String, password: String) async throws {
        let response = try await WAPSupabase.shared.client.auth.signUp(email: email, password: password)
        KeychainHelper.save(key: KeychainHelper.Keys.authToken, value: response.user.id.uuidString)
    }

    static func signIn(email: String, password: String) async throws {
        let session = try await WAPSupabase.shared.client.auth.signIn(email: email, password: password)
        KeychainHelper.save(key: KeychainHelper.Keys.authToken, value: session.user.id.uuidString)
    }
